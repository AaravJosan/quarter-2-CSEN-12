...
/*
    ...
    Description: Helper function for the partition function. swaps two variables
*/
...
/*
    ...
    Description: divides the array into two based on the pivot
*/


static int partition(SET *sp, void **arr, int low, int high) {
    void *pivot = arr[high];
    int i = (low - 1);

    for (int j = low; j <= high - 1; j++) {
       if((*sp->compare)(arr[j], pivot) < 0) {
            i++;
            swap(&arr[i], &arr[j]);
        }
...
static void quickSort(SET *sp, void **arr, int low, int high) {
    if (low < high) {
        int pi = partition(sp, arr, low, high);

        quickSort(sp, arr, low, pi - 1);
        quickSort(sp, arr, pi + 1, high);
    }
}
...
/*
    ...
    Description: returns a sorted array of all the elements
*/
